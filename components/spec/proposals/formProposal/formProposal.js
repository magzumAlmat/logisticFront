...
function FormCompany({
  ...
  const { refetch: refetchProposals } = useGetAllSpecProposalsQuery();
  const { refetch: refetchProposal } = useGetSpecProposalQuery(form.id);
  const { data: labels } = useGetCompanyLabelsQuery();
  const [deleteExpense] = useDeleteSpecExpenseMutation();
      ...
      {(role === "manager" || role === "admin") && (
        <>
          ...
          <TextField
            ...
            id="outlined-size-normal"
            value={form?.declared_dshv || ""}
            onChange={(e) =>
              setForm({ ...form, declared_dshv: e.target.value })
            }
          />
          ...
          (!newProposal && role === "manager") ? (
            <>
              ...
              <TextField
                ...
                id="outlined-size-normal"
                value={form?.actual_dshv || ""}
                onChange={(e) =>
                  setForm({ ...form, actual_dshv: e.target.value })
                }
              />
            </>
          ) : null}
