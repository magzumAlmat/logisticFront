...
import { kppLabels, statusLabels, typeOptions } from "@/data/basket";
...
function BasketForm({
  ...
  const { refetch } = useGetSpecBasketQuery();

  const { data } = useGetSpecCompanyCarLabelsQuery();
  const [createBasket, { isSuccess, error, isLoading }] =
    useCreateSpecBasketMutation();
  const [deleteBasketCar] = useDeleteSpecBasketCarMutation();
